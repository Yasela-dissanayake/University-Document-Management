...
def create_semester_workflow(
    ...
    """Create a new semester workflow record"""
...
def get_semester_workflow(ipfs_hash: str) -> Optional[Dict]:
    """Get workflow status for a semester record"""
    with sqlite3.connect(WORKFLOW_DB) as conn:
...
def approve_semester_workflow(
    ...
    """Approve a semester workflow, moving it to next state"""
    ...
    return {
        ...
        "next_approver": "DEAN" if next_state == "PENDING_DEAN" else None
    }
...
def get_pending_approvals(role: str) -> List[Dict]:
    ...
    with sqlite3.connect(WORKFLOW_DB) as conn:
        ...
        return [{
            ...
            "submitted_by": row["submitted_by"],
            "submitted_at": row["submitted_at"],
            "waiting_days": (int(time.time()) - row["submitted_at"]) // 86400
        } for row in rows]
